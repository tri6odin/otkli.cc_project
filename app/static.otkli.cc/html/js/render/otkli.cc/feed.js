if (!userData) {
  ...
  SEND('page page_rows cus_status', 'GET', 'feed').then(
    result => {
      if (result[0] == 200) {
        if (result[1] == '[]') {
          document.querySelector("grid[id=grid_main]").classList.add("fullwidth")
          document.querySelector("grid[id=grid_main]").innerHTML = `<column class="m" style="align-items:center;margin: var(--len-m) auto var(--len-m) auto">
  ...
  function findButton() {
    ...
    SEND('page page_rows cus_status', 'GET', 'feed').then(
      result => {
        if (result[0] == 200) {
          document.querySelector("grid[id=grid_main]").innerHTML = "";
          document.querySelector("#loadmorebtn").classList.remove("loading")
          document.querySelector("grid[id=grid_main]").classList.remove("fullwidth")
          if (result[1] == '[]') {
            document.querySelector("grid[id=grid_main]").classList.add("fullwidth")
            document.querySelector("grid[id=grid_main]").innerHTML = `<column class="m" style="align-items:center;margin: var(--len-m) auto var(--len-m) auto">
            <icon class='i-notfound' style="font-size:var(--font-xxl);color:var(--gray)"></icon>
  ...
  function renderVacancyGrid(that,replace_status) {
    let cloneTemplateVac = tmplVac.content.cloneNode(true)
    let parent = document.querySelector('grid[id=grid_main]')
    for (const [key, value] of Object.entries(JSON.parse(that))) {
      let cloneTemplate = cloneTemplateVac.cloneNode(true)
      ...
      console.log(value.vacancy_card.busyness);
